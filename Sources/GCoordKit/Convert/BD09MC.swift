...
class BD09MCConvert {
    static let MCBAND: [Double] = [12890594.86, 8362377.87, 5591021, 3481989.83, 1678043.12, 0]
    static let LLBAND: [Double] = [75, 60, 45, 30, 15, 0]
    static let MC2LL = [
        [
    ...
    private static func transform(x: Double, y: Double, factors: [Double]) -> Position {
        let cc = abs(y) / factors[9]
        
        var xt = factors[0] + factors[1] * abs(x)
        var yt = factors[2]
        yt += factors[3] * cc
        yt += factors[4] * pow(cc, 2)
        yt += factors[5] * pow(cc, 3)
        yt += factors[6] * pow(cc, 4)
        yt += factors[7] * pow(cc, 5)
        yt += factors[8] * pow(cc, 6)
        
        xt *= x < 0 ? -1 : 1
        yt *= y < 0 ? -1 : 1
        
        return (lng: xt, lat: yt)
    }

    public static func BD09toBD09MC(coord: Position) -> Position {
        let (lng, lat) = coord
        var factors: [Double] = []
        
        for i in 0..<LLBAND.count {
            if abs(lat) > LLBAND[i] {
                factors = LL2MC[i]
                break
            }
        ...
        return transform(x: lng, y: lat, factors: factors)
    }

    public static func BD09MCtoBD09(coord: Position) -> Position {
        let (lng, lat) = coord
        var factors: [Double] = []
        
        for i in 0..<MCBAND.count {
            if abs(lat) >= MCBAND[i] {
                factors = MC2LL[i]
                break
            }
